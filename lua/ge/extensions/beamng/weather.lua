...
local M = {}
local weather_presets = require('weather_presets')
...
local function playAmbientSound(sound_name, duration)
  soundPlay = true
  soundDuration = duration
  soundTimer = duration
  soundName = sound_name
  Engine.Audio.playOnce('AudioGui', 'art/sound/environment/' .. sound_name)
end
...
local function activate(presetName)
  ...
  for objClassStr, attribTable in pairs(p) do
    ...
    if objClassStr == "Sound" then
      playAmbientSound(attribTable.name, attribTable.duration)
      goto continue
    end
...
local function onPreRender(dt)
  if soundPlay then
    ...
    if soundTimer <= 0 then
      playAmbientSound(soundName, soundDuration)
    end
  end
...
local function loadPresets()
  ...
  for name, v in pairs(weather_presets) do
    loadPreset(name, v)
  end
end
...
local function onExtensionLoaded()
  ...
  log('I', 'weather', "CUSTOM Weather module loaded")
  loadPresets()
end
